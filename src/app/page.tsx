    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-12">
        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/20 text-slate-700 text-xs font-black uppercase tracking-wider mb-4 border border-primary/30">
            <Activity className="h-3 w-3" /> Clinical Systems Portal
          </div>
          <h1 className="text-4xl md:text-7xl font-black font-headline tracking-tighter text-slate-800 max-w-4xl leading-[0.9]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <Card className="group hover:shadow-2xl transition-all border-none overflow-hidden relative shadow-sm">
            <div className="absolute top-0 left-0 w-full h-1.5 bg-slate-600" />
            <CardContent className="p-10 flex flex-col items-center text-center space-y-5">
              <div className="h-20 w-20 bg-slate-600 rounded-2xl flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                <Stethoscope className="h-10 w-10" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black font-headline uppercase tracking-tight text-slate-800">Clinic Entry</h3>
          ...
          <Card className="group hover:shadow-2xl transition-all border-none overflow-hidden relative shadow-sm">
            ...
            <CardContent className="p-10 flex flex-col items-center text-center space-y-5">
              <div className="h-20 w-20 bg-primary rounded-2xl flex items-center justify-center text-slate-800 group-hover:scale-110 transition-transform">
                <ShieldCheck className="h-10 w-10" />
              </div>
              <div className="space-y-2">
                <h3 className="text-2xl font-black font-headline uppercase tracking-tight text-slate-800">Admin Dashboard</h3>
                <p className="text-muted-foreground text-sm font-medium">
                  Secure access for administrators to review logs, manage users, and view AI insights.
                </p>
              </div>
              <Button asChild className="w-full bg-primary hover:bg-primary/90 text-slate-800 font-bold h-12 mt-4 gap-2">
                <Link href="/login">
